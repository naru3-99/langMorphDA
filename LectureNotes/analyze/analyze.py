...
def get_important_process2(target_keyword_ls):
    # execveの引数にtarget_keywordが存在するもの
    # そのプロセスの子プロセスを分けて取得
    rmrf('./important_proc2/')
    cp_syscall_ls = ["56-clone" ,"57-fork","58-vfork","435-clone3"]
    for path in get_all_file_path_in('./output/'):
        target_path_ls = []
        target_name = ""
        ...
        if (flag):
            ...
            for row in contents_str.split('\n'):
                ...
                if (len(tsp )<2):
                    continue
                if (tsp[1] in cp_syscall_ls):
                    target_path_ls.append(f'./output/{tsp[2]}.csv')
            ensure_path_exists(f"./important_proc2/{target_name}/")
            for tpath in target_path_ls:
                if is_exists(tpath):
                    save_str_to_file(load_str_from_file(tpath),f'./important_proc2/{target_name}/{get_file_name(tpath)}')
...
if __name__ == '__main__':
    ...
    for i,path in enumerate(get_all_file_path_in('./prepi_count')):
        save_str_to_file(load_str_from_file(path),f'./result/{i}.csv')
        if i == 199:
            break

    all_cnt_ls = []
    for path in get_all_file_path_in('./result/'):
        all_cnt = int(load_str_from_file(path).split('\n')[0].split(',')[-1])
        all_cnt_ls.append(all_cnt)
    print(sum(all_cnt_ls)/200)
    print(max(all_cnt_ls))
    print(min(all_cnt_ls))


    get_important_process2(['python3','import_numpy_only.py'])
    for path in get_all_dir_path_in('./important_proc2/'):
        count_syscall_num_all('./prepi_count2/',path,path.split('/')[-2])

    ensure_path_exists('./result2/')
    for i,path in enumerate(get_all_file_path_in('./prepi_count2/')):
        save_str_to_file(load_str_from_file(path),f'./result2/{i}.csv')
        if i == 199:
            break

    all_cnt_ls = []
    for path in get_all_file_path_in('./result2/'):
        all_cnt = int(load_str_from_file(path).split('\n')[0].split(',')[-1])
        all_cnt_ls.append(all_cnt)
    print(sum(all_cnt_ls)/200)
    print(max(all_cnt_ls))
